	...
	<div class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
		...
		<div class="row">
			...
			foreach ( $customer_orders->orders as $customer_order ) {
				...
				?>
				<div class="col-md-6 ">
				<div class="woocommerce-cart-form-left-box woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?> order">
					<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
						<div class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php elseif ( 'order-number' === $column_id ) : ?>
								<span class="order-icon"><i class="icon-bill"></i>سفارش</span>
								<a href="<?php echo esc_url( $order->get_view_order_url() ); ?>">
									<?php echo esc_html( _x( '#', 'hash before order number', 'woocommerce' ) . $order->get_order_number() ); ?>
								</a>

							<?php elseif ( 'order-date' === $column_id ) : ?>
								<span class="order-icon"><i class="icon-calendar"></i>تاریخ</span>
								<time datetime="<?php echo esc_attr( $order->get_date_created()->date( 'c' ) ); ?>"><?php echo esc_html( wc_format_datetime( $order->get_date_created() ) ); ?></time>

							<?php elseif ( 'order-status' === $column_id ) : ?>
							<span class="order-icon"><i class="icon-pending"></i>وضعیت</span>
								<?php echo esc_html( wc_get_order_status_name( $order->get_status() ) ); ?>
                           
							<?php elseif ( 'order-total' === $column_id ) : ?>
							<span class="order-icon"> <i class="icon-basket"></i>مجموع</span>
