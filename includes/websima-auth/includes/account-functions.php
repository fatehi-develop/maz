                    ...
                    if($_SESSION['websima_auth']['status'] == 'registered'){
                        if($_SESSION['websima_auth']['user_id']){
                            ...
                            if(in_array('customer',$user_data->roles)){
								...
								$websima_auth_captcha_checker = websima_auth_captcha_checker();
								if($websima_auth_captcha_checker or (!$websima_auth_captcha_checker and !is_wp_error($user_check))){
...
function websima_auth_captcha_checker(){
	$captcha_check = false;
	if(class_exists('WP_CAPTCHA')){
		$c4wp_default_settings = get_option('c4wp_default_settings');	
		if($c4wp_default_settings){
			if($c4wp_default_settings['c4wp_options']['enable_form_settings']['wp_login_form'] == 1){
				$captcha_check = true;
			}
		}
	}
