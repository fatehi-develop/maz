...
function websima_delivery_before_order_notes($checkout){
    $exclude_shipping_method = websima_delivery_shipping_method_exclude();
    $chosen_methods = WC()->session->get('chosen_shipping_methods');
    $chosen_shipping = $chosen_methods[0];
    if(!empty($exclude_shipping_method) and in_array($chosen_shipping,$exclude_shipping_method)){ $exclude_status = 'yes'; }else{ $exclude_status = 'no'; }

    echo '<div class="delivery-wrapper '.(($exclude_status == 'yes') ? 'd-none' : '').'">';
        ...
        woocommerce_form_field( 'delivery_day', array(
            ...
            'required'    => (($exclude_status == 'yes') ? false : true),
            'options'     => $day_options
        ),
...
function websima_delivery_view_order_and_thankyou_page($order_id){
    ...
    if($delivery_day or $delivery_time){
        echo '<div class="delivery-wrap">';
            ...
            echo '<ul>';
                 if($delivery_day){
                     echo '<li><strong><i class="icon-tarikhe-tahvil"></i>تاریخ تحویل سفارش: </strong><span class="d-inline-block">'.esc_html($delivery_day).'</span></li>';
                 }
                 if($delivery_time){
                     $delivery_time_array = explode("-",$delivery_time);
                     $start = $delivery_time_array[0];
                     $end = $delivery_time_array[1];
                     $final_time = 'از '.esc_html($start).' تا '.esc_html($end);
                     echo '<li><strong><i class="icon-saate-tahvil"></i>ساعت تحویل سفارش: </strong><span>'.esc_html($final_time).'</span></li>';
                 }
            echo '</ul>';
        echo '</div>';
    }
...
add_action('woocommerce_init','websima_delivery_woocommerce_init');
function websima_delivery_woocommerce_init(){
    $shipping_methods = WC()->shipping->get_shipping_methods();
    foreach($shipping_methods as $shipping_method) {
        add_filter('woocommerce_shipping_instance_form_fields_' . $shipping_method->id, 'websima_delivery_shipping_form_add_fields');
    }
}

function websima_delivery_shipping_form_add_fields($settings){
    $settings['shipping_delivery_switch'] = [
        'title' => 'روز و ساعت تحویل سفارش',
        'type' => 'checkbox',
        'label' => 'فعال',
    ];
...
function websima_delivery_shipping_method_exclude(){
    $exclude_array = array();
    foreach(WC()->session->get('shipping_for_package_0')['rates'] as $method_id => $rate){
        $original_rate_id = $rate->id;

        $rate_id = str_replace(":","_",$rate->id);
        $rate_id = 'woocommerce_'.$rate_id.'_settings';

        $rate_value = get_option($rate_id);
        if(!empty($rate_value)){
            if($rate_value['shipping_delivery_switch'] == 'no'){
                $exclude_array[] =  $original_rate_id;
            }
        }
    }
...
add_action('wp_ajax_websima_delivery_check_fields_visibility', 'websima_delivery_check_fields_visibility');
add_action('wp_ajax_nopriv_websima_delivery_check_fields_visibility', 'websima_delivery_check_fields_visibility');
function websima_delivery_check_fields_visibility(){
    $allowed_html   =   array();
    $shipping_method = wp_kses($_POST['shipping_method'] ,$allowed_html);
    if($shipping_method){
        $rate_id = str_replace(":","_",$shipping_method);
        $rate_id = 'woocommerce_'.$rate_id.'_settings';

        $rate_value = get_option($rate_id);
        if(!empty($rate_value)){
            if($rate_value['shipping_delivery_switch'] == 'yes'){
                $status = 'show';
            }else{
                $status = 'hide';
            }
